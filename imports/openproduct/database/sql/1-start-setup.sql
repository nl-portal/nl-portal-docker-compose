...
VALUES (1, 'Notification API', 'nrc', 'http://localhost:8002/api/v1/', 'valtimo_client', 'e09b8bc5-5831-4618-ab28-41411304309d', 'zgw', '', '', '', '', '', null, null, '415042b2-c740-4532-be88-38ac5f8c9060', 10, '', 'httplocalhost8001catalogiapiv1', 43200 );
...
-- add externe verwijzing config (doesn't exist in 1.5.0)
...
-- Set sequence for zgw_consumers_service
...
-- Set sequence for notifications_api_common_notificationsconfig
SELECT pg_catalog.setval(
               'public.notifications_api_common_notificationsconfig_id_seq',
               COALESCE((SELECT MAX(id) FROM public.notifications_api_common_notificationsconfig), 1),
               true
       );
...
SELECT pg_catalog.setval(
               ...
               COALESCE((SELECT MAX(id) FROM public.producttypen_externeverwijzingconfig), 1),
               true
       );
...
SELECT pg_catalog.setval(
               ...
               COALESCE((SELECT MAX(id) FROM public.producttypen_dmnconfig), 1),
               true
       );